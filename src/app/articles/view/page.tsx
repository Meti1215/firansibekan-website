...
function ArticleViewPageContent() {
          ...
          <div className="max-w-4xl mx-auto">
            <div className="bg-white rounded-sm border border-gray-100 shadow-lg p-10">
              <div className="flex items-center gap-3 text-gray-500">
                <div className="w-5 h-5 border-2 border-[#0F2A4A] border-t-transparent rounded-sm animate-spin" />
                Loading article...
              </div>
            </div>
          </div>
        ...
        <>
            ...
            <div className="absolute bottom-0 left-0 right-0 px-4 sm:px-6 lg:px-8 pb-5 md:pb-10">
              ...
              >
                <div className="flex items-center gap-2 mb-4">
                  <span className="inline-flex items-center px-3 py-1 text-xs font-bold text-white bg-[#0F2A4A] rounded-sm shadow-lg">
            ...
            >
              <div className="bg-white rounded-sm md:rounded-sm shadow-lg border border-gray-100 px-3 py-2.5 md:px-6 md:py-4 flex flex-wrap items-center justify-between gap-2 md:gap-4">
                <div className="flex items-center gap-2 md:gap-4">
                  <div className="w-8 h-8 md:w-11 md:h-11 rounded-sm bg-gradient-to-br from-[#0F2A4A] to-[#0F2A4A] flex items-center justify-center shadow-md">
          ...
          <section className="px-4 sm:px-6 lg:px-8 py-10">
            ...
            >
              ...
              {article.excerpt && (
                <p className="text-sm md:text-xl text-gray-600 leading-relaxed mb-4 md:mb-8 font-medium border-l-4 border-[#0F2A4A] pl-3 md:pl-5">
                  {article.excerpt}
                </p>
              )}
              ...
              {article.content && article.content.includes('<') ? (
                <div
                  className="prose prose-lg prose-gray max-w-none
                    ...
                    prose-a:text-[#0F2A4A] prose-a:no-underline hover:prose-a:underline
                    prose-strong:text-gray-900
                    prose-blockquote:border-l-[#0F2A4A] prose-blockquote:bg-[#F3F4F6] prose-blockquote:py-1 prose-blockquote:px-2 prose-blockquote:rounded-r-lg prose-blockquote:not-italic
            ...
            <div className="max-w-3xl mx-auto">
              <div className="border-t border-gray-200 pt-6 flex items-center justify-between">
                <Button
                  variant="ghost"
                  onClick={() => (window.location.href = '/articles/')}
                  className="text-gray-600 hover:text-[#0F2A4A]"
                >
                  <ArrowLeft className="w-4 h-4 mr-2" />
                  Back to All News
                </Button>
              ...
              <div className="max-w-5xl mx-auto">
                <div className="border-t border-gray-200 pt-6 md:pt-12">
                  <h2 className="text-lg md:text-2xl font-bold text-gray-900 mb-4 md:mb-8">
                    More in <span className="text-[#0F2A4A]">{article.category}</span>
                  </h2>
                    ...
                    {relatedArticles.map((related, index) => (
                      ...
                      >
                        ...
                        <div className="p-2.5 md:p-4">
                          <h3 className="font-bold text-gray-900 text-[10px] md:text-sm leading-tight md:leading-snug group-hover:text-[#0F2A4A] transition-colors line-clamp-2 mb-1 md:mb-2">
                            {related.title}
                          </h3>
                          <p className="text-gray-500 text-[9px] md:text-xs leading-snug md:leading-relaxed line-clamp-2">
                            {related.excerpt}
                          </p>
                          <span className="inline-flex items-center gap-1 text-xs font-semibold text-[#0F2A4A] mt-3 group-hover:gap-2 transition-all">
                            Read More
...
export default function ArticleViewPage() {
  return (
    <React.Suspense fallback={<div className="min-h-screen flex items-center justify-center bg-gray-50">Loading...</div>}>
